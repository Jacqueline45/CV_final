...
import numpy as np
from data import cfg_mnet, cfg_re50, cfg_snet, cfg_mbnetv3
...
parser.add_argument('--dataset_folder', default='../face_detection/CV_dataset/val/images/', type=str, help='dataset path')
...
if __name__ == '__main__':
    ...
    open(args.txt_pth, "w").close() # clear the txt file
    ...
    for i, img_pth in enumerate(test_dataset):
        ...
        # --------------------------------------------------------------------
        # save_name = args.save_folder + "val_solution.txt"
        # dirname = os.path.dirname(save_name)
        # if not os.path.isdir(dirname):
        #     os.makedirs(dirname)
        with open(args.txt_pth, "a") as fd:
            bboxs = dets
            ...
            for box in bboxs:
                ...
                landmas = " ".join([str(box[j]) for j in range(5, len(box))])
                line = str(x) + " " + str(y) + " " + str(w) + " " + str(h) + " " + confidence + " " + landmas + "\n"
                fd.write(line)
